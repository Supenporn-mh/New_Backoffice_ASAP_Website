...
const mockAsapReady = ref([
  {
    id: 1,
    title: 'ASAP พร้อมให้บริการ - รูปที่ 1',
    imageDesktop: {
      th: 'https://placehold.co/1280x500/FF595A/white?text=ASAP+Ready+Desktop+TH+1',
      en: 'https://placehold.co/1280x500/FF595A/white?text=ASAP+Ready+Desktop+EN+1',
      zh: 'https://placehold.co/1280x500/FF595A/white?text=ASAP+Ready+Desktop+ZH+1'
    },
    imageAltDesktop: { th: 'ASAP พร้อมให้บริการ', en: 'ASAP Ready to Serve', zh: 'ASAP随时为您服务' },
    imageMobile: {
      th: 'https://placehold.co/390x300/FF595A/white?text=ASAP+Ready+Mobile+TH+1',
      en: 'https://placehold.co/390x300/FF595A/white?text=ASAP+Ready+Mobile+EN+1',
      zh: 'https://placehold.co/390x300/FF595A/white?text=ASAP+Ready+Mobile+ZH+1'
    },
    imageAltMobile: { th: 'ASAP พร้อมให้บริการ', en: 'ASAP Ready to Serve', zh: 'ASAP随时为您服务' },
    order: 1,
    isActive: true
  },
  {
    id: 2,
    title: 'ASAP พร้อมให้บริการ - รูปที่ 2',
    imageDesktop: {
      th: 'https://placehold.co/1280x500/2574FF/white?text=ASAP+Ready+Desktop+TH+2',
      en: 'https://placehold.co/1280x500/2574FF/white?text=ASAP+Ready+Desktop+EN+2',
      zh: 'https://placehold.co/1280x500/2574FF/white?text=ASAP+Ready+Desktop+ZH+2'
    },
    imageAltDesktop: { th: 'ASAP พร้อมให้บริการ 2', en: 'ASAP Ready to Serve 2', zh: 'ASAP随时为您服务 2' },
    imageMobile: {
      th: 'https://placehold.co/390x300/2574FF/white?text=ASAP+Ready+Mobile+TH+2',
      en: 'https://placehold.co/390x300/2574FF/white?text=ASAP+Ready+Mobile+EN+2',
      zh: 'https://placehold.co/390x300/2574FF/white?text=ASAP+Ready+Mobile+ZH+2'
    },
    imageAltMobile: { th: 'ASAP พร้อมให้บริการ 2', en: 'ASAP Ready to Serve 2', zh: 'ASAP随时为您服务 2' },
    order: 2,
    isActive: true
  }
])
...
const mockAppBanner = ref({
  ...
  bannerImage: 'https://placehold.co/600x400/FF595A/white?text=App+Banner+Desktop',
  bannerImageMobile: 'https://placehold.co/390x360/5a5a6e/white?text=App+Banner+Mobile',
...
const mockCheckIns = ref([
  ...
  {
    id: 6,
    bookingNumber: 'ASP20241214002',
    bookingDate: '2024-12-14',
    customerName: 'ธนากร มีสุข',
    phone: '091-777-8888',
    ...
    returnTime: '12:00',
    checkInDate: '2024-12-14T11:00:00',
    status: 'confirmed',
    documents: [
      {
        label: 'สำเนาบัตรประชาชน',
        type: 'image/jpeg',
        url: 'https://placehold.co/800x500/2574FF/white?text=ID+Card+6',
        fileName: 'id_card_6.jpg'
      },
      {
        label: 'สำเนาใบขับขี่',
        type: 'image/jpeg',
        url: 'https://placehold.co/800x500/2574FF/white?text=License+6',
        fileName: 'driving_license_6.jpg'
      }
    ],
    actionBy: 'Admin',
    actionDate: '2024-12-14T13:00:00',
    rejectReason: null
  },
  {
    id: 5,
...
export const useApi = () => {
  ...
  const getAsapReadyItems = async () => {
    return [...mockAsapReady.value].sort((a, b) => a.order - b.order)
  }

  const getAsapReadyItem = async (id) => {
    return mockAsapReady.value.find(item => item.id === parseInt(id))
  }

  const createAsapReadyItem = async (data) => {
    const newId = mockAsapReady.value.length > 0
      ? Math.max(...mockAsapReady.value.map(i => i.id)) + 1
      : 1
    const newItem = { ...data, id: newId }
    mockAsapReady.value.push(newItem)
    return newItem
  }

  const updateAsapReadyItem = async (id, data) => {
    const index = mockAsapReady.value.findIndex(i => i.id === parseInt(id))
    if (index !== -1) {
      mockAsapReady.value[index] = { ...mockAsapReady.value[index], ...data }
      return mockAsapReady.value[index]
    }
    return null
  }

  const deleteAsapReadyItem = async (id) => {
    const index = mockAsapReady.value.findIndex(i => i.id === parseInt(id))
    if (index !== -1) {
      mockAsapReady.value.splice(index, 1)
      return true
    }
    return false
  }

  const reorderAsapReadyItems = async (newOrder) => {
    newOrder.forEach((id, index) => {
      const item = mockAsapReady.value.find(i => i.id === id)
      if (item) item.order = index + 1
    })
    mockAsapReady.value.sort((a, b) => a.order - b.order)
    return true
  }
